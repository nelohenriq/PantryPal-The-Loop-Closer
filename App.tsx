...
import { Search, ShoppingBag, ChefHat, Sparkles, MapPin, Filter, Store, Database, Settings } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col pb-24 md:pb-0">
      ...
      <header className="bg-white sticky top-0 z-30 border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <h1 className="text-xl font-bold text-gray-900 tracking-tight hidden sm:block">PantryPal</h1>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-lg relative">
            <input 
              ...
              placeholder="What are you craving? (e.g., Spicy Tacos)"
              className="w-full pl-10 pr-4 py-2.5 bg-gray-100 border-transparent focus:bg-white focus:ring-2 focus:ring-emerald-500 rounded-xl transition text-sm"
            />
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 pt-6">
        ...
        {!recipes.length && !isGenerating && (
          <div className="flex flex-col items-center justify-center mt-12 text-center max-w-2xl mx-auto">
            <div className="w-16 h-16 bg-gradient-to-tr from-emerald-400 to-cyan-500 rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-emerald-200">
               <Sparkles className="text-white w-8 h-8" />
            </div>
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Let's close the dinner loop.</h2>
            <p className="text-gray-500 mb-8 text-lg">
              Tell us what you have, and we'll tell you what to cook—or exactly where to buy the missing piece.
            </p>

            <div className="w-full max-w-md">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Try these</p>
              <div className="flex flex-wrap justify-center gap-2">
                {CUISINE_SUGGESTIONS.map(c => (
                  <button 
                    key={c}
                    onClick={() => { setCuisineQuery(c); handleSearch(); }}
                    className="px-4 py-2 bg-white border border-gray-200 rounded-full text-gray-600 hover:border-emerald-500 hover:text-emerald-600 transition shadow-sm"
                  >
                    {c}
                  </button>
                ))}
        ...
        {isGenerating && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
             {[1,2,3].map(i => (
               <div key={i} className="h-96 bg-white rounded-2xl border border-gray-200 p-4 animate-pulse">
        ...
        {matchedRecipes.length > 0 && !isGenerating && (
           <div className="flex items-center gap-3 mb-6 overflow-x-auto pb-2 scrollbar-hide">
              ...
              {['All', 'Cook Now', 'Easy', 'Medium', 'Hard'].map(filter => {
                ...
                if (filter === 'Cook Now') {
                    if (isActive) {
                        className += 'bg-emerald-600 text-white shadow-md shadow-emerald-200';
                    } else {
                    ...
                    if (isActive) {
                        ...
                        className += 'bg-white border border-gray-200 text-gray-600 hover:border-gray-300 hover:bg-gray-50';
                    }
                }
            ...
            {filteredRecipes.some(r => r.matchScore >= 0.8) && (
              <section>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">Cook Now</h2>
                  <span className="text-sm text-gray-500 font-normal ml-2">You have almost everything</span>
                </div>
            ...
            {filteredRecipes.some(r => r.matchScore < 0.8) && (
              <section className="pt-4 border-t border-gray-200">
                <div className="flex items-center gap-2 mb-4 mt-4">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">Worth a Quick Run</h2>
                  <span className="text-sm text-gray-500 font-normal ml-2">A few ingredients missing</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredRecipes.filter(r => r.matchScore < 0.8).map(match => (
                    <RecipeCard 
      ...
      <div className="md:hidden fixed bottom-6 right-6 flex flex-col gap-3 z-40">
        ...
        <button 
            onClick={() => setShowStoreKB(true)}
            className="w-12 h-12 bg-blue-600 text-white rounded-full shadow-lg shadow-blue-200 hover:bg-blue-700 transition flex items-center justify-center"
        >
            <Database className="w-5 h-5" />
        </button>
        <button 
            onClick={() => setShowPantry(true)}
            className="relative w-14 h-14 bg-emerald-600 text-white rounded-full shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition flex items-center justify-center"
        >
